...
Declare_Any_Class( "Tetrahedron",              // A demo of flat vs smooth shading.  Also our first 3D, non-planar shape.
  ...
  // *********** CUSTOM ***********
Declare_Any_Class( "Hat",              //1 square 4 triangles
  { 'populate'() 
      {
        var a = 1/Math.sqrt(3);
             
        this.positions     .push( vec3(-1,0,-1), vec3(1,0,-1), vec3(-1,0,1), vec3(1,0,1) ); // square base of hat
        ...
        this.positions.push( vec3(0,0,1), vec3(1,0,0), vec3(0,1,0) );
        this.normals.push( vec3( a,a,a), vec3( a,a,a), vec3( a,a,a) );
        this.texture_coords.push( vec2(0,0), vec2(1,0), vec2(1,0) );    
        this.indices.push( 4, 5, 6);     

        this.positions.push( vec3(0,0,1), vec3(-1,0,0), vec3(0,1,0) );        //negative in the x
        this.normals.push( vec3( -a,a,a), vec3( -a,a,a), vec3( -a,a,a) );
        this.texture_coords.push( vec2(0,0), vec2(1,0), vec2(1,0) );      
        this.indices.push( 7, 8, 9);  

        this.positions.push( vec3(0,0,-1), vec3(1,0,0), vec3(0,1,0) );      //negative in the z
        this.normals.push( vec3( a,a,-a), vec3( a,a,-a), vec3( a,a,-a) );
        this.texture_coords.push( vec2(0,0), vec2(1,0), vec2(1,0) );      
        this.indices.push( 10, 11, 12);  

        this.positions.push( vec3(0,0,-1), vec3(-1,0,0), vec3(0,1,0) );      //negative in the x and z
        this.normals.push( vec3( -a,a,-a), vec3( -a,a,-a), vec3( -a,a,-a) );
        this.texture_coords.push( vec2(0,0), vec2(1,0), vec2(1,0) );      
...
//////////////////custom shape///////////////////////////////////////////////////
Declare_Any_Class( "Hat_Head",     // Combine a hat with a grid sphere
  { populate( rows, columns ) 
      { 
       Subdivision_Sphere    .prototype.insert_transformed_copy_into( this, [ 3 ], mult( scale(.75, .75, .75), translation( 0, -1, 0 )) );    
        Hat                  .prototype.insert_transformed_copy_into( this, [ rows, columns ] ); 
      } 
  }, Shape )
//////////////////////////////////////////////////////////////////////////////////
