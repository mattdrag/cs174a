...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 'construct'( context )
    ...
    'init_keys'( controls )   // init_keys():  Define any extra keyboard shortcuts here
      { 
        controls.add( "f",     this, function() { this.camera_state = 1; } );
        controls.add( "g",     this, function() { this.camera_state = 0; } );
        controls.add( "n",     this, function() { alert(this.object_map[0].x); } );
        controls.add( "j",     this, function() { this.attempt_move(3); } );
        controls.add( "i",     this, function() { this.attempt_move(1); } );
        controls.add( "l",     this, function() { this.attempt_move(4); } );
        controls.add( "k",     this, function() { this.attempt_move(2); } );
      },
    'create_map'( )
      {
        this.add_object(0, 120, 1);   
        this.add_object(0, 100, 1); 
        this.add_object(0, 80, 1); 
        this.add_object(0, 60, 1); 
        this.add_object(0, 40, 1); 
        this.add_object(0, 20, 1); 
        this.add_object(0, 0, 1); 
        this.add_object(0, -20, 1); 
        this.add_object(0, -40, 1); 
        this.add_object(0, -60, 1); 
      },
    'add_object'( xv, zv, type )
      {
        this.object_map.push({x: xv, z: zv, t: type});
      },
    'attempt_move'( dir )
      {             
        var nx;
    ...
    'draw_wall'( model_transform, graphics_state, x_coord, z_coord)
      { 
        ...
        this.shapes.box       .draw( graphics_state, model_transform,                      this.red_clay );
    ...
    'draw_player'( model_transform, graphics_state, x, z )
      { 
        ...
        model_transform = mult( model_transform, translation( x, -10, z ) );
        //draw 8 tree segments
        var count;
        for(count = 0; count < 8; count++) {
          model_transform = this.draw_player_segment( model_transform, graphics_state );
    ...
    'draw_player_segment'( model_transform, graphics_state )
      { 
        model_transform = mult( model_transform, translation( 0, -1, 0 ) );
    ...
    'display'( graphics_state ) 
        ...
        graphics_state.lights = [ new Light( vec4(  this.player_x,  10,  this.player_z, 1 ), Color( 1, .4, 0, 1 ), 900) ];   // Arguments to construct a Light(): Light source position or 
...
Declare_Any_Class( "Surfaces_Tester",
  { 'construct'( context )
      { context.globals.animate = true;
        var shapes = { 'good_sphere' : new Subdivision_ ( 4 ),
                       'box'         : new Cube(),
                       'strip'       : new Square(),
