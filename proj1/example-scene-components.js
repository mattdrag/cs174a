      ...
      { var model_transform = identity();             // We have to reset model_transform every frame.
        ...
        **********************************/                                     // From here on down it's just some example shapes drawn 
        ...
        var t = graphics_state.animation_time/1000,   tilt_spin   = rotation( 700*t, [          .1,          .8,             .1 ] ),
                                                      funny_orbit = rotation(  360*t, [ 0, Math.cos(t), 0 ] );
...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 'construct'( context )
    ...
    'draw_ground'( model_transform, graphics_state )
      { 
        ...
        this.shapes.box       .draw( graphics_state, model_transform,                      this.greenGround );
    ...
    'draw_tree'( model_transform, graphics_state )
      { 
        ...
        //draw 8 tree segments
        var count;
        for(count = 0; count < 8; count++) {
          model_transform = this.draw_tree_segment( model_transform, graphics_state );
          model_transform = mult( model_transform, translation( 0, 1, 0 ) );
        }
        //now draw top of tree
        model_transform = mult( model_transform, translation( 0, 1, 0 ) );
    ...
    'draw_tree_segment'( model_transform, graphics_state )
      { 
        model_transform = mult( model_transform, translation( 0, -1, 0 ) );
        model_transform = mult( model_transform, rotation(4 * Math.cos(graphics_state.animation_time / 1600), 0, 0, 1) );
        model_transform = mult( model_transform, translation( 0, 1, 0 ) );
        model_transform = mult( model_transform, scale( .25, .5, .25 ) );
        this.shapes.box       .draw( graphics_state, model_transform,                      this.brown_clay );
        model_transform = mult( model_transform, scale( 4, 2, 4 ) );
    ...
    'draw_bee'( model_transform, graphics_state ) 
      { 
        ...
        //rotate bee around y origin
        model_transform = mult( model_transform, rotation(graphics_state.animation_time * .02, 0, -1, 0) );

        //move bee up and down
        model_transform = mult( model_transform, translation( -2, 2 * Math.cos(graphics_state.animation_time / 1600), 6 ) );

        //origin of bee - stagger bee on Z axis
        model_transform = mult( model_transform, translation( -2, 1, 2 ) );

        //draw head, body, wings, legs, then tail
        //head
        ...
        //wings
        model_transform = this.draw_wings( model_transform, graphics_state );

        //legs
        //save position for later
        var pre_leg_position = model_transform;
        //starting at front, draw 3 pairs of 2 legs, left and right side
        model_transform = mult( model_transform, translation( -.5, 0, 0 ) );
        var count;
        for(count = 0; count < 3; count++) {
          // left
          model_transform = this.draw_leg( model_transform, graphics_state, 1 );
          // right
          model_transform = this.draw_leg( model_transform, graphics_state, -1 );
          //translate 
          model_transform = mult( model_transform, translation( .5, 0, 0 ) );
        ...
        //reset position
        model_transform = pre_leg_position;
    ...
    'draw_wings'( model_transform, graphics_state )
      { 
        var origin = model_transform;

        model_transform = mult( model_transform, translation( 0, .5, .5 ) );
        model_transform = mult( model_transform, rotation(64 * Math.cos(graphics_state.animation_time / 1600) , 1, 0, 0) );
        model_transform = mult( model_transform, translation( 0, -.5, -.5 ) );

        //left wing
        model_transform = mult( model_transform, translation( 0, .5, 2 ) );
        model_transform = mult( model_transform, scale( .5, .125, 1.5 ) );
        this.shapes.box       .draw( graphics_state, model_transform,                      this.greyPlastic );
    ...
    'draw_leg'( model_transform, graphics_state, orientation)
      { 
        var origin = model_transform;
        

        //leg - main
        model_transform = mult( model_transform, translation( 0, -1, .5 * orientation ) );
        model_transform = mult( model_transform, translation( 0, .5, 0 ) );
        model_transform = mult( model_transform, rotation(20 * Math.cos(graphics_state.animation_time / 1600) * orientation, 1, 0, 0) );
        model_transform = mult( model_transform, translation( 0, -.5, 0 ) );
        model_transform = mult( model_transform, scale( .125, .5, .125 ) );
        this.shapes.box       .draw( graphics_state, model_transform,                      this.greyPlastic );
        ...
        model_transform = mult( model_transform, translation( 0, -1, 0 ) );
        model_transform = mult( model_transform, translation( 0, .5, 0 ) );
        model_transform = mult( model_transform, rotation(25 * orientation + 20 * Math.cos(graphics_state.animation_time / 1600) * orientation, 1, 0, 0) );
        model_transform = mult( model_transform, translation( 0, -.5, 0 ) );
        model_transform = mult( model_transform, scale( .125, .5, .125 ) );
        this.shapes.box       .draw( graphics_state, model_transform,                      this.greyPlastic );
    ...
    'draw_scene'( model_transform, graphics_state ) //Attempts to draw all objects in the scene
      { 
        model_transform = this.draw_ground( model_transform, graphics_state );
        model_transform = this.draw_tree( model_transform, graphics_state );
    ...
    'display'( graphics_state ) 
        ...
        model_transform = this.draw_scene( model_transform, graphics_state ); // Calls head of draw function
      }
  }, Scene_Component );
